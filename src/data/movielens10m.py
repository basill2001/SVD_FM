...
class Movielens10m:
    ...
    def train_test_getter(self):
        train=pd.read_csv('dataset/ml-10m/ratings.dat',sep='::',header=None, names=['user_id','movie_id','rating','timestamp'],encoding='latin-1')
        train=train.sort_values(by=['user_id','timestamp'])
        tqdm.pandas()
        train_data=train.groupby('user_id').progress_apply(lambda x: x.iloc[:int(len(x)*0.7)])
        test_data=train.groupby('user_id').progress_apply(lambda x: x.iloc[int(len(x)*0.7):])
    ...
    def user_getter(self):
        ...
        user_info=pd.read_csv('dataset/ml-10m/users.csv',encoding='latin-1')
