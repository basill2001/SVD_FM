...
class Preprocessor:
    ...
    def preprocess(self, args, train_df, item_info , user_info, ui_matrix):
        """
        Method to preprocess the input data
        :return: Preprocessed data
        """ 
        # Negative Sampling
        ns = NegativeSampler(args, train_df, item_info, user_info)
        ns_sampled_df = ns.negativesample(args.isuniform)
        ...
        user_embedding, item_embedding = SVD(args).fit_truncatedSVD(ui_matrix)
    ...
    def merge_embedding(self, user_embedding: np.ndarray, item_embedding: np.ndarray, ns_sampled_df: pd.DataFrame):
    ...
    def label_encode(self, args, cat_columns, cont_columns):
        ...
        self.le_dict = {}
        # when we use SVD, we don't need to embed user_id and item_id
